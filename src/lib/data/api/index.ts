...
async function get_json(endpoint: string) {
	const response = await fetch(build_url(endpoint));
	return response.json();
}

async function post_json(endpoint: string, body?: unknown) {
...
async function patch_json(endpoint: string, body?: unknown) {
	if (!is_logged_in()) throw new UnauthenticatedError();
	const headers = build_headers();
	const response = await fetch(build_url(endpoint), {
		method: 'PATCH',
...
async function delete_json(endpoint: string, body?: unknown) {
	if (!is_logged_in()) throw new UnauthenticatedError();
	const headers = build_headers();
	const response = await fetch(build_url(endpoint), {
		method: 'DELETE',
