<script lang="ts">
	import ToolTip from './ToolTip.svelte';

	let {
		type: typeInput = 'filled',
		variant = 'default',
		functionType = 'button',
		icon = undefined,
		iconSize = 20,
		iconColor = 'currentColor',
		href = '',
		target = '',
		label = '',
		disabled = false,
		toolTipText = undefined,
		style = '',
		children,
		onclick
	}: {
		type?: 'filled' | 'tonal' | 'text' | 'outlined' | 'icon';
		variant?: 'default' | 'danger' | 'onDangerBackground';
		functionType?: typeof HTMLButtonElement.prototype.type;
		icon?: any | undefined;
		iconSize?: number;
		iconColor?: string;
		href?: string;
		target?: string;
		label?: string;
		disabled?: boolean;
		toolTipText?: string | undefined;
		style?: string;
		children?: any;
		onclick?: (e: MouseEvent) => void;
	} = $props();

	const type = $derived(children ? typeInput : 'icon');
	const Icon = $derived(icon);
</script>

<ToolTip content={toolTipText} html={false}>
	{#if href}
		<a {href} {target} aria-label={label} class={`${type} ${variant}`} class:disabled>
			{#if Icon}
				<Icon size={iconSize} color={iconColor} />
			{/if}
			{@render children?.()}
		</a>
	{:else}
		<button
			onclick={onclick}
			class={`${type} ${variant}`}
			style="{style}"
			class:disabled
			aria-label={label}
			type={functionType}
			{disabled}
		>
			{#if Icon}
				<Icon size={iconSize} color={iconColor} />
			{/if}
			{@render children?.()}
		</button>
	{/if}
</ToolTip>

<style lang="scss">
	a,
	button {
		min-width: max-content;
		font-size: 0.95rem;
		text-decoration: none;
		color: var(--text-one);
		font-weight: 600;
		border: none;
		border-radius: 100px;
		display: flex;
		justify-content: center;
		align-items: center;
		gap: 0.5rem;
		cursor: pointer;
		transition:
			transform 0.4s var(--bezier-one),
			filter 0.4s var(--bezier-one);
		user-select: none;
		padding: 16px 24px;

		&:hover:not(.disabled) {
			filter: brightness(85%);
		}

		&.disabled {
			filter: grayscale(100%);
			cursor: not-allowed;
		}

		&.filled {
			background-color: var(--primary);
			color: var(--text-three);
		}

		&.tonal {
			background-color: var(--surface-four);
		}

		&.text {
			background-color: transparent;
			color: var(--primary);
			font-weight: 500;
			padding: 0;
		}

		&.outlined {
			border: 2px solid var(--primary);
			background-color: transparent;
		}

		&.icon {
			&:hover {
				filter: brightness(75%);
			}
			background-color: transparent;
			color: currentColor;
			padding: 0;
		}

		&.danger {
			background-color: var(--red-one);
			color: var(--surface-four);
		}

		&.onDangerBackground {
			background-color: #ffd3d3;
			color: #601410;
		}
	}
</style>
